...
if echo "${1}" | grep -q "${PROJECT_DIR}/input" && [[ $(find "${INPUTDIR}" -maxdepth 1 -type f -size +10M -print | wc -l) -gt 1 ]]; then
	...
	if echo "${1}" | grep -q -e '^\(https\?\|ftp\)://.*$' >/dev/null; then
		...
		if echo "${URL}" | grep -q "mega.nz\|mediafire.com\|drive.google.com"; then
			...
			aria2c -x16 -s8 --console-log-level=warn --summary-interval=0 --check-certificate=false "${URL}" || aria2c -x16 -s8 --console-log-level=warn --summary-interval=0 --check-certificate=false "${URL}" || {
...
if [[ -s "${PROJECT_DIR}"/.github_token ]]; then
	...
	[ -e ".gitattributes" ] && {
		...
		git push -u origin "${branch}" > /dev/null 2>&1 || git push -u origin "${branch}" > /dev/null 2>&1 || git push -u origin "${branch}" > /dev/null 2>&1 || { echo "Can't push to github.com" && exit 1; }
	}
        echo "Dumping extras..."
	...
	do
		...
		[ -e ".gitattributes" ] && {
			...
			git add ".gitattributes" > /dev/null 2>&1
			git commit -sm "Setup Git LFS" > /dev/null 2>&1
			git push -u origin "${branch}" > /dev/null 2>&1 || git push -u origin "${branch}" > /dev/null 2>&1 || git push -u origin "${branch}" > /dev/null 2>&1 || { echo "Can't push to ${GITLAB_INSTANCE}" && exit 1; }
		}
                echo "Dumping extras..."
		git add -- . ':!system/' ':!vendor/' > /dev/null 2>&1
		git commit -sm "Add extras for ${description}" > /dev/null 2>&1
